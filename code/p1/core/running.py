...
from core.qlearning import do_batch_qlearning
...
def run_multiple_trials(env, 
    ...
    losses_file = os.path.join(
        dumps_dir, 'losses_%s.csv' % (model.name))

    results = None
    results_file = os.path.join(
            dumps_dir, 'results_%s.csv' % (model.name))
    ...
    for lr in learning_rates:
        ...
        for r in range(num_runs):
            ...
            cur_losses, cur_means = do_batch_qlearning(env, replay_buffer, model, lr)
